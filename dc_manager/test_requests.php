...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["approved"])) {
    $id = $_GET["approved"];
    // cancel the test data from the database
    $sql = "UPDATE test_requests SET status='Approved' WHERE id=$id";
    if (mysqli_query($conn, $sql)) {
        // Redirect to the desired page after successful deletion
        header("Location: test_requests.php");
        exit();
...
<body>
            ...
            <tbody>
                <?php
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '<td>';
                        echo '<a href="test_requests.php?cancel=' . $row['id'] . '"class="btn btn-danger"><i class="fa fa-times"></i></a>';
                        echo '&nbsp;';
                        echo '<a href="test_requests.php?approved=' . $row['id'] . '"class="btn btn-primary"><i class="fa fa-check"></i></a>';
                        echo '</td>';
    ...
    <div class="modal fade" id="add_request_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
